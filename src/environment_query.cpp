...
Ref<CQueryResult> CEnvironmentQuery::request_query()
{
    ...
    query_items.clear();
    UtilityFunctions::print_rich("Child count: ", get_child_count());
    ...
    for (Variant child : get_children())
    {
        ...
        if (generator != nullptr)
        {
            ...
            UtilityFunctions::print_rich("Items after generation: ", query_items.size());
            generator->perform_tests(query_items);
            UtilityFunctions::print_rich("Items after tests: ", query_items.size());
        }
        else
        {
            UtilityFunctions::print_rich("Child is not a CQueryGenerator3D");
        }
    }
